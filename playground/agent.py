...
class TextQNetwork(nn.Module):
    def __init__(self, model_name="distilbert-base-uncased", hidden_size=64):
        ...
        self.advantage = nn.Sequential(
            nn.Linear(hidden_size, 32), nn.ReLU(), nn.Linear(32, 4)
        )
...
class ShrineAgent:
    def __init__(self, state_size, action_size):
        ...
        self.optimizer = optim.AdamW(
            self.qnetwork.parameters(), lr=0.0001, weight_decay=0.01
        )
    ...
    def get_state_representation(self, observation):
        ...
        stats = torch.tensor(
            [observation["stats"]], dtype=torch.float16, device=self.device
        )
        items = torch.tensor(
            [observation["items"]], dtype=torch.float16, device=self.device
        )
    ...
    def replay(self):
        ...
        with torch.autocast(device_type="cuda"):
            ...
            state_stats = torch.tensor(
                [s["stats"] for s in states], dtype=torch.float16, device=self.device
            )
            state_items = torch.tensor(
                [s["items"] for s in states], dtype=torch.float16, device=self.device
            )

            next_state_texts = [s["text"] for s in next_states]
            next_state_stats = torch.tensor(
                [s["stats"] for s in next_states],
                dtype=torch.float16,
                device=self.device,
            )
            next_state_items = torch.tensor(
                [s["items"] for s in next_states],
                dtype=torch.float16,
                device=self.device,
            )
            ...
            next_q_values = self.target_network(
                next_state_texts, next_state_stats, next_state_items
            )
            ...
            loss = nn.MSELoss()(
                current_q_values.gather(1, actions.unsqueeze(1)).squeeze(), targets
            )
