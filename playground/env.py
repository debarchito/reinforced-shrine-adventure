...
class ReinforcedShrineAdventureEnv(gym.Env):
    ...
    def update_variables_from_choice(self, choice_text: str):
        ...
        elif "Play it safe" in choice_text or "Follow the original beach route" in choice_text:
            self.took_beach_path = True
    ...
    def calculate_reward(self) -> float:
        ...
        if not self.took_beach_path:
            prepared = (self.items["has_talisman"] and 
                       self.items["has_first_aid_kit"] and 
                       self.stats["caution"] >= 2)
            if prepared:
                reward += 2.0
            else:
